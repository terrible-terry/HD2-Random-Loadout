...
const HorizontalBarChart = forwardRef(
  ({ icon= { color: "info", component: "" }, title="", description="", height="10rem", chart, maxX }, ref) => {
  ...
  const { data, options } = configs(chart.labels || [], chartDatasets, maxX);
  ...
  const renderChart = (
    <MDBox py={2} pr={2} pl={icon.component ? 1 : 2}>
      ...
      {useMemo(
        () => (
          <MDBox height={height}>
            <Bar ref={ref} id="CuttingLayout" data={data} options={options} redraw />
          </MDBox>
        ),
        [chart, height, ref]
      )}
